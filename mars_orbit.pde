...
float center = 400; // Center of drawing
...
void plotEarth(float angle1, float angle2, float marsAngle1, float marsAngle2, float radius, float origin){
  ...
  float y1 = origin + radius*sin(radians(angle1));
  fill(0,76,153);
  stroke(0,76,153);
  ellipse(x1,y1,15,15);
  ...
  float y2 = origin + radius*sin(radians(angle2));
  fill(0,76,153);
  stroke(0,76,153);
  ellipse(x2,y2,15,15);
...
//  line(o,o, xIntercept, 0);
  ...
  float distance = (side * sin(radians(earthAngle)))/sin(radians(intersectionAngle));
//  if(distance < 200){
//    distance = -distance;
//  }
  println(distance);
